...
static nxt_int_t njs_module_relative_path(njs_vm_t *vm, nxt_str_t *path,
    njs_module_info_t *info);
static nxt_int_t njs_module_realpath_compare(nxt_str_t *file1,
    nxt_str_t *file2);
...
njs_parser_module(njs_vm_t *vm, njs_parser_t *parser)
{
    ...
    if (njs_module_realpath_compare(&prev->file, &info.file) == NXT_OK) {
        njs_parser_syntax_error(vm, parser, "Cannot import itself \"%V\"",
                                name);
        goto fail;
    }

    ret = njs_lexer_init(vm, &lexer, &info.file, body.start,
...
static nxt_int_t
njs_module_realpath_compare(nxt_str_t *file1, nxt_str_t *file2)
{
    char  path1[MAXPATHLEN], path2[MAXPATHLEN];

    realpath((char *) file1->start, path1);
    realpath((char *) file2->start, path2);
